...
@section ('title', trans('labels.backend.quiz.polls.management') . ' | ' . trans('labels.backend.quiz.polls.edit'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.quiz.polls.management') }}
        <small>{{ trans('labels.backend.quiz.polls.edit') }}</small>
    </h1>
@endsection

@section('content')
    {!! Form::model($poll, ['route' => ['admin.quiz.polls.update', $poll->id], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) !!}

    <div class="box box-success">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.quiz.polls.edit') }}</h3>
        ...
        <div class="box-body">
            <div class="form-group">
                {!! Form::label('category_id', trans('validation.attributes.backend.quiz.polls.category'), ['class' => 'col-lg-2 control-label']) !!}
                <div class="col-lg-10">
                    {!! Form::select('category_id', $categories->lists('name', 'id'), null, ['class' => 'form-control', 'placeholder' => trans('validation.attributes.backend.quiz.polls.category_hint')]) !!}
                </div>
            </div><!--form control-->
        </div><!-- /.box-body -->
    ...
    <div class="box box-info">
        <div class="box-body">
            <div class="pull-left">
                <a href="{{route('admin.quiz.polls.index')}}" class="btn btn-danger btn-xs">{{ trans('buttons.general.cancel') }}</a>
            </div>
